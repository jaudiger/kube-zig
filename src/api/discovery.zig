...
pub const DiscoveryClient = struct {
    ...
    pub fn deinit(self: *DiscoveryClient) void {
        self.mu.lock();
    ...
    pub fn coreAPIVersions(self: *DiscoveryClient) !Client.ApiResult(std.json.Parsed(types.MetaV1APIVersions)) {
        return self.client.get(types.MetaV1APIVersions, "/api", self.ctx);
    }
    ...
    pub fn apiGroups(self: *DiscoveryClient) !Client.ApiResult(std.json.Parsed(types.MetaV1APIGroupList)) {
        self.client.logger.debug("fetching api groups", &.{});
        return self.client.get(types.MetaV1APIGroupList, "/apis", self.ctx);
    }

    /// Fetch all resources in the core API group v1 (GET /api/v1).
    /// Returns pods, services, nodes, configmaps, etc.
    pub fn coreResources(self: *DiscoveryClient) !Client.ApiResult(std.json.Parsed(types.MetaV1APIResourceList)) {
        self.client.logger.debug("fetching core resources", &.{});
        return self.client.get(types.MetaV1APIResourceList, "/api/v1", self.ctx);
    }
    ...
    pub fn groupResources(self: *DiscoveryClient, group: []const u8, version: []const u8) !Client.ApiResult(std.json.Parsed(types.MetaV1APIResourceList)) {
        ...
        return self.client.get(types.MetaV1APIResourceList, path, self.ctx);
    }
    ...
    pub fn hasResource(self: *DiscoveryClient, group: []const u8, version: []const u8, resource: []const u8) !bool {
        if (try self.ensureResourcesCached(group, version) == .not_found) return false;
        self.mu.lock();
        defer self.mu.unlock();
    ...
    pub fn hasGroup(self: *DiscoveryClient, group: []const u8) !bool {
        if (group.len == 0) return true;
        try self.ensureGroupsCached();
        self.mu.lock();
        defer self.mu.unlock();
    ...
    pub fn findPreferredVersion(self: *DiscoveryClient, out_allocator: std.mem.Allocator, group: []const u8) !?[]const u8 {
    ...
    pub fn resourceMeta(
        ...
        if (try self.ensureResourcesCached(group, version) == .not_found) return null;
        self.mu.lock();
        defer self.mu.unlock();
    ...
    /// Return null if the resource is not found.
    pub fn isResourceNamespaced(self: *DiscoveryClient, group: []const u8, version: []const u8, resource: []const u8) !?bool {
        if (try self.ensureResourcesCached(group, version) == .not_found) return null;
        self.mu.lock();
        defer self.mu.unlock();
        const r = self.findResourceInCacheLocked(group, version, resource) orelse return null;
    ...
    pub fn invalidateCache(self: *DiscoveryClient) void {
        self.mu.lock();
        defer self.mu.unlock();
        self.clearGroupsCacheLocked();
        self.clearAllResourcesCacheLocked();
    ...
    pub fn invalidateGroupsCache(self: *DiscoveryClient) void {
        self.mu.lock();
        defer self.mu.unlock();
        self.clearGroupsCacheLocked();
    }
    ...
    pub fn invalidateResourceCache(self: *DiscoveryClient, group: []const u8, version: []const u8) void {
    ...
    fn ensureGroupsCached(self: *DiscoveryClient) !void {
        if (self.options.cache_ttl_ns == 0) {
            ...
            switch (result) {
                .ok => |parsed| {
                    ...
                    self.groups_cache = .{
                        .parsed = parsed,
                        .fetched_at = std.time.Instant.now() catch unreachable,
                    };
                    return;
                },
                .api_error => |e| {
        ...
        // Check if cache is valid.
        {
            ...
            if (self.groups_cache) |cached| {
                if (isCacheValid(cached.fetched_at, self.options.cache_ttl_ns)) return;
            }
        }
    ...
    fn ensureResourcesCached(self: *DiscoveryClient, group: []const u8, version: []const u8) !EnsureResult {
        const key = try self.makeCacheKey(group, version);
        defer self.allocator.free(key);

        if (self.options.cache_ttl_ns == 0) {
            // Caching disabled: fetch, populate, return.
            const result = try self.fetchResources(group, version);
            switch (result) {
                .ok => |parsed| {
                    self.mu.lock();
                    defer self.mu.unlock();
        ...
        // Check if cache is valid.
        {
            ...
            if (self.resources_cache.get(key)) |cached| {
                if (isCacheValid(cached.fetched_at, self.options.cache_ttl_ns)) return .cached;
            }
        }
    ...
    fn evictIfNeededLocked(self: *DiscoveryClient) void {
    ...
    fn evictExpiredLocked(self: *DiscoveryClient) void {
        if (self.options.cache_ttl_ns == 0) return;

        var i: usize = self.resources_cache.count();
        while (i > 0) {
            i -= 1;
            if (!isCacheValid(self.resources_cache.values()[i].fetched_at, self.options.cache_ttl_ns)) {
    ...
    fn isCacheValid(fetched_at: std.time.Instant, ttl_ns: u64) bool {
        const now = std.time.Instant.now() catch return false;
        return now.since(fetched_at) < ttl_ns;
    }
    ...
    fn fetchResources(self: *DiscoveryClient, group: []const u8, version: []const u8) !Client.ApiResult(std.json.Parsed(types.MetaV1APIResourceList)) {
        if (group.len == 0) {
            return self.coreResources();
        }
        return self.groupResources(group, version);
    }
};
...
test "MetaV1APIResourceList: parse sample JSON with subresource" {
    ...
    try testing.expect(std.mem.indexOf(u8, parsed.value.resources[1].name, "/") != null);
}
